...
def get_data():
    ...
    with hc.conn.cursor() as cur:
        ...
        ## SQL Query
        sql = "select * from keybox ORDER BY id ASC"
...
def update_status_data(locker_id, borrower_id, status):
    with hc.conn.cursor() as cur:
        ...
        #print(time_updated)
        sql = f"UPDATE keybox SET borrower_id = %s, date_updated = %s, time_updated = %s, status = %s WHERE id = %s"
        ...
        hc.conn.commit()
        if (borrower_id != ""):
            print(f"Key#{locker_id} is borrowed by {borrower_id}")
        else:
            print(f"Key#{locker_id} is returned and free now")
        ## Get cloumn name
        #name = [desc[0] for desc in cur.description]
